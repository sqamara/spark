   ...
   */
   private def newOrUsedShuffleStage(
   	...
   	if (mapOutputTracker.containsShuffle(shuffleDep.shuffleId)) {
   		...
   		taskGraph.addRepeatedShuffleStage(shuffleDep.shuffleId, stage.id)  // are shuffleDep.shuffleId's the same as the stage id???? <- NO!
            ...
            resultStage.activeJob match {
            	...
            	if (!job.finished(rt.outputId)) {
                  ...
                  if (job.numFinished == job.numPartitions) {
                  	...
                  	taskGraph.addStage(resultStage, getParentStages(resultStage.rdd, resultStage.firstJobId))
